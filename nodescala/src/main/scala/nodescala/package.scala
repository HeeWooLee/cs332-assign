...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      def recurseFlatMap(fs: List[Future[T]], acc: List[T]): Future[List[T]] = {
        fs match {
          case head :: tail => head.flatMap(f => recurseFlatMap(tail, acc:::List(f)))
          case Nil => Future(acc)
        }
      }
      recurseFlatMap(fs, List())
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]()
      p completeWith Future.firstCompletedOf(fs)
      p.future
    }
